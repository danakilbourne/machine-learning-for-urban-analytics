...
knitr::opts_chunk$set(echo = TRUE, cache = F, cache.lazy = FALSE) # notice cache=T here
knitr::opts_chunk$set(fig.height=4, fig.width=7, fig.align = 'center', warning = F)

if(!require('pacman')) {
  install.packages('pacman')
}
pacman::p_load(keras3, ggplot2, glmnet, RColorBrewer, wordcloud, neuralnet,
               plotly, latex2exp, data.table, reticulate, kerastuneR)

# for Apple M1 chip and if using conda
use_condaenv('r')
...
**If you have trouble installing tensorflow or if you want to use GPU, you can try [Google Colab](http://colab.research.google.com). It supports R.**
You can copy [this notebook](https://drive.google.com/file/d/1x0uKtAU7ufUvZUwibdtrk4Yk_-tki8PV/view?usp=sharing) to your Google drive and then run the .ipynb on Colab.
Note that you need to change runtime type to R. You can select GPU there as well.
...
- Gaussian Error Linear Unit (GELU): similar to ReLU but it is non-convex and non-monotonic compared to ReLU. It is often used in transformer models.
		$$f = x \Phi(x)$$
where $\Phi(x)$ is the cumulative density function of standard normal distribution.
...
act_func <- c("tanh", "Sigmoid", "ReLU", "Leaky ReLU", "GELU")
act_func_df <- data.frame(
  method = factor(rep(act_func, each = x_len), 
               levels = act_func, ordered = T),
  x = x,
  y = c(tanh_y, sigmoid_y, relu_y, relu_leaky_y, gelu)
  )
...
* **Softmax classifier**: one of the most popular choices. Denote the scores in the output layer as $(s_1, \ldots, s_K)$. Then the loss function is 
  $$L = - \frac{1}{n}\sum_{i=1}^n \sum_{j=1}^K 1 y_{ij} \log\Bigg(\frac{e^{s_{y_{ij}}}}{\sum_j e^{s_{y_{ij}} }} \Bigg)$$
...
Finally, we need to specify the activation function. Here we use the *rectified linear unit* or ReLU function mentioned earlier.  (Relu(x)= $\max(0, X)$).
...
# hist(weights[[1]])   # W^(1)  #dim(weights[[1]])
...
**Get the labels:**
```{r}
# get the k_argmax of the predicted probabilities
y.pred <- model %>% predict(data3_xval[1:5,]) %>% op_argmax(axis = 2) %>% as.integer() # majority vote!
...
We have introduced Neural network and immplemented it using keras. Neural network provides a non-linear model to predict response variables. They can be categorical or continuous responses. 
...
Since there are so many tuning parameters in our NN, how to determine the best tuning parameters are important. Here are the tuning parameters:
...
[kerastuneR](https://eagerai.github.io/kerastuneR/index.html) provides us an easy and clean way for hypertuning using [keras-tuner](https://keras.io/guides/keras_tuner/getting_started/) package.

```{r eval = F}
# First install kerastuneR R package
# install.packages("kerastuneR")
devtools::install_github('eagerai/kerastuneR')
# Install the keras-tuner Python package
reticulate::py_install("keras-tuner", envname = 'r')
```

To tune the model architecture, we need to specify the search space for each element. 
Let's say we use one hidden layer and tune the number of neurons and the learning rate.

```{r}
library(kerastuneR)

# If "RecursionError: maximum recursion depth exceeded in comparison kerastuner", then
# sys <- reticulate::import("sys")
# sys$setrecursionlimit(as.integer(100000))

build_model <- function(hp) {
	
	# specify the search space for units 
	# Int stands for integer
	# Use hp$Float for numbers
	l1_units_grid = hp$Int('l1_units',
										min_value=32,
										max_value=128,
										step=32)
	
	# specify the learning rates
	# Choice for discrete choices 
	lr_grid = hp$Choice('learning_rate', values=c(1e-2, 1e-3, 1e-4))
	
	# specify the whole architecture
	model = keras_model_sequential() %>% 
		layer_dense(units=5,
								input_shape = c(p),
								activation='relu') %>% 
		...
		compile(
			optimizer= tf$keras$optimizers$Adam(lr_grid),
  		loss = "sparse_categorical_crossentropy",
			metrics='accuracy') 
...
Then we build our tuner using `RandomSearch()`. There are two other tuners `BayesianOptimization` and `Hyperband`, which correspond to different tuning algorithms.

```{r}
# Define a Random Search tuner for hyperparameter optimization
# The tuner will use the build_model function to create and evaluate models
# Objective is to maximize validation accuracy
# Perform at most 5 trials of hyperparameter optimization
# Execute each trial 3 times to reduce result variance
# Save the results and logs in the 'output' directory
# Name the tuning project as 'yelp_NN'
# Do not overwrite the results if a project with the same name exists
tuner <- RandomSearch(
    build_model,
...
```{r}
tuner %>% fit_tuner(data3_xtrain, data3_ytrain, 
                    epochs = 5, 
                    validation_split = .15 # set 15% of the data3_xtain, data3_ytrain as the validation data
)
```
...
Let's look at the tuning results.
...
```{r}
results_summary(tuner, num_trials = 3)
```

Based on this tuning, we know that the best model includes 32 neurons in the hidden layer and the best learning rate is 0.0001

```{r}
best_2_models <- tuner %>% get_best_models(1) 
best_model <- best_2_models[[1]]
```

Now let's retrain the best model using the entire training data.

```{r}
best_model %>% fit(data3_xtrain, data3_ytrain, epochs = 6)
```

Comparing the NN with 2 hidden layers above, `best_model` using only 1 hidden layer is comparable with similar testing error.
    
```{r}
results <- best_model %>% evaluate(data3_xval, data3_yval)
results
```
...
Let's take a look at the first 30 images in the training set. `purrr` as part of the `tidyverse` is a useful/convenient package handling functions. 
```{r echo=TRUE}
index <- 1:30
...
```{r}
train_images_vec <- array_reshape(train_images, c(60000, 28*28)) #correct shape to what is expected by keras
dim(train_images_vec)
```

Instead of using `array_reshape()` to reshape, we can directly use `layer_flatten()` in our architecture to vectorize the image as we will see later. We will first scale each pixel from 0 to 255 to 0 to 1.
...
How do we find the minimizer $\theta^* = 0$ using gradient descent? The **goal is to find a point such that its derivative is 0**. We use the gradient/derivative to guide us with direction that moves to the minimal. We take a small step every time.
...
gradient_descent_trace <- function(x = NULL, y = NULL, init, gradient, step = 1e-1, eps = 1e-5, max_iteration = 1e5) {
  ...
  i <- 1
  while(max(abs(gradient(theta))) > eps & i < max_iteration){
